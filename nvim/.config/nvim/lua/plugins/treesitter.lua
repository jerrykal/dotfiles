return {
  ...
  --     tsc.setup(opts)
  --
  --     -- codediff.nvim opens its diff/explorer in a dedicated tab. Per-buffer
  --     -- matching can't catch it (one pane is the real working-tree file, with a
  --     -- normal name + filetype), so detect the codediff tab by any of its marked
  --     -- buffers and toggle context globally on entering/leaving that tab.
  --     local function in_codediff_tab()
  --       for _, win in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
  --         local buf = vim.api.nvim_win_get_buf(win)
  --         local name = vim.api.nvim_buf_get_name(buf)
  --         if vim.bo[buf].filetype:match("^codediff") -- explorer / help panels
  --           or name:match("^codediff://") -- virtual git-file buffers
  --           or vim.fn.fnamemodify(name, ":t"):match("^CodeDiff ") -- diff scratch buffers
  --         then
  --           return true
  --         end
  --       end
  ...
  --       group = vim.api.nvim_create_augroup("codediff_no_context", { clear = true }),
  --       callback = function()
  --         local want = not in_codediff_tab()
