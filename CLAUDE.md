...
- Every machine shares the folder as **Send & Receive** (Syncthing's [standard folder type](https://docs.syncthing.net/users/foldertypes.html) — each device both sends *and* receives changes), so sync is bidirectional: an edit on a remote flows back to the laptop too. "Laptop is the source of truth" is therefore a *convention* (it holds `.git`; you commit there), **not** something the folder type enforces. Editing the same file on two machines before they sync yields a `*.sync-conflict-*` copy (older mtime loses, and the copy propagates like any other file) — but because `.stignore` keeps `.git` out of the sync, a conflict can never corrupt git internals. (Both machines must be online at the same time to sync — Syncthing has no async drop-box like GitHub.)
- `.stfolder`/`.stversions` are Syncthing's own runtime markers and are gitignored.

Bringing up a remote: install Syncthing (`brew "syncthing"` is in the Brewfile), pair it with the laptop, accept the shared `~/.dotfiles` folder as **Send & Receive**, then run `./install.sh --skip-deps` (or `just link`) once to create the Stow symlinks — stow doesn't need git, so the absent `.git` is fine. `just sync-id` prints a machine's device ID for pairing. To bootstrap a machine the git way instead, clone the repo and run `./install.sh`.
